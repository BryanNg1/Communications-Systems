...
gp_in = [gp1 gp1(2:no_sample1) gp1]; %3 cycles of the triangular wave
figure(1)
...
for n = nvec
    ...
    absn = abs(n);
    if(n > 0)
      c_in(m) = 1/2*((sin(pi*n/2))/(pi*n/2));
    elseif (n == 0)
      c_in(m) = 0.5;
    else 
      c_in(m) = 1/2*((sin(pi*absn/2))/(pi*absn/2));
    end
end
...
Hf = 1 ./((w.^2)+(1.414.*w)+1);%filter transfer function
